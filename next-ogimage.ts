...
async function capturePage(browser: Browser, route: Route) {
  await getMeta(browser, route)
    .then(async () => {
      ...
      if (!fs.existsSync(getPath(config.outputFolder))) {
        await promises.mkdir(getPath(config.outputFolder));
      }

      await page.screenshot({
        type: "png",
        path: `${config.outputFolder}/${routeToFilename(route)}.png`,
      });
...
function useOgImage() {
  ...
  if (pathname == "/") return `${config.outputFolder}/index.png`;

  return `${config.outputFolder}/${pathname}.png`;
}

function useMetadata() {
  const { query } = useRouter();

  return { title: query.title, description: query.description };
}
